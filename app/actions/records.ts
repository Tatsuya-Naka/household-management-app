...
export async function appliedToRecords({ type, currency, country, date, regular_unit, items, recordId, payment, status }: RecordsProps, formState: RecordsState, formData: FormData): Promise<RecordsState> {
    "use server";
    ...
    if (type === "expenses") {
        ...
        await db.record.update({
            ...
            data: {
                ...
                isSubmitted: true, totalcost: totalCost,
                // object:  
            }
        });
        ...
        await db.record.update({
            ...
            data: {
                ...
                isSubmitted: true, resourceId: findResource.id, income_categoryId: catId, income_amount: data.income, income_status: data.status,
                // object:  
            }
        });
...
export async function savedToRecords(formState: RecordsState, formData: FormData): Promise<RecordsState> {
    return {
        errors: {}
    };
}